...
import { claudeQuery, type QueryOptions } from './claude/provider-fastapi.js';
...
async function runSetup(): Promise<void> {
  ...
  while (true) {
    ...
    if (isHeadlessLinux) {
      ...
      // macOS / Windows / GUI Linux: generate QR PNG and output URL
      console.log(`QR_URL:${qrcodeUrl}`);
      const QRCode = await import('qrcode');
      const pngData = await QRCode.toBuffer(qrcodeUrl, { type: 'png', width: 400, margin: 2 });
      writeFileSync(QR_PATH, pngData);
    ...
    try {
      accountData = await waitForQrScan(qrcodeId);
      console.log('✅ 绑定成功!');
      break;
    } catch (err: any) {
  ...
  config.workingDirectory = config.workingDirectory || process.cwd();
  saveConfig(config);

  // 输出账号信息供 app.py 读取
  if (accountData) {
    console.log(`SETUP_RESULT:${JSON.stringify({ accountId: accountData.accountId, userId: accountData.userId })}`);
  }

  console.log('✅ 设置完成，正在启动服务...');
}
...
async function runDaemon(accountId?: string, port?: number): Promise<void> {
  const config = loadConfig();
  const account = accountId ? loadAccount(accountId) : loadLatestAccount();
  ...
  // -- 本地发送接口，供 FastAPI 主动推送消息 --
  const localSendPort = port ?? config.localSendPort ?? 3001;
  const localSendToken = config.apiToken || '';
  const httpServer = createServer((req, res) => {
    // FastAPI uses this endpoint to confirm that a newly rebound bridge is
    // actually listening and that it loaded the expected account.  Binding
    ...
    if (req.method !== 'POST' || req.url !== '/local/send') {
  ...
  httpServer.listen(localSendPort, '127.0.0.1', () => {
    console.log(`[本地发送接口] 监听 127.0.0.1:${localSendPort}`);
  });
...
async function handleMessage(
  ...
  if (userText.startsWith('/')) {
    // /bind 用户名 密码 — 绑定系统账号
    if (userText.startsWith('/bind ')) {
      const parts = userText.slice(6).trim().split(/\s+/);
      if (parts.length < 2) {
        await sender.sendText(fromUserId, contextToken, '用法：/bind 用户名 密码');
        return;
      }
      ...
      try {
        const resp = await fetch(`${apiUrl}/api/weixin/bind`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ weixin_id: fromUserId, username, password, token: apiToken }),
        });
        const data = await resp.json();
        await sender.sendText(fromUserId, contextToken, data.msg || (data.ok ? '绑定成功' : '绑定失败'));
      } catch (e) {
        await sender.sendText(fromUserId, contextToken, '绑定请求失败，请检查服务是否运行');
      }
      return;
    }
...
async function sendToClaude(
  ...
  try {
    ...
    // 发送 typing indicator（不留消息，替代"正在处理"文字提示）
    try {
      const configResp = await api.getConfig(fromUserId, contextToken);
      if (configResp.typing_ticket) {
        await api.sendTyping(fromUserId, configResp.typing_ticket, 1);
      } else {
        await sender.sendText(fromUserId, contextToken, '⏳ 正在处理，请稍候...');
      }
    } catch {
      await sender.sendText(fromUserId, contextToken, '⏳ 正在处理，请稍候...');
    }
    ...
    // FastAPI 模式不使用 session resume，直接清除避免干扰
    ...
    if (result.text) {
      ...
      await sender.sendText(fromUserId, contextToken, '⚠️ 处理请求时出错，请稍后重试。');
    } else if (!anySent) {
      await sender.sendText(fromUserId, contextToken, 'ℹ️ 未返回内容（可能因权限被拒而终止）');
    }
...
// Parse CLI args: node main.js [setup] [--account <id>] [--port <n>]
const args = process.argv.slice(2);
const command = args[0] === 'setup' ? 'setup' : 'start';
...
const cliAccountId = getArg('--account');
const cliPort = getArg('--port') ? parseInt(getArg('--port')!, 10) : undefined;
...
if (command === 'setup') {
  ...
  runDaemon(cliAccountId, cliPort).catch((err) => {
